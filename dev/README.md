...
covering `rack1..rack4` over `c1..c160` and `gpu1..gpu2` over `g1..g80`, by
...
The dashboard's ten panels, in provisioned order:
...
| 10 | A declaration too small for what arrived | Label `rack`, `rack1` declared at four slots | Prometheus, the full 240-node cluster |

Panels 1-4 are the same 20-node CSV, grouped four ways, and need nothing
running but Grafana. Panels 5-10 read this dev cluster's live Prometheus:
