...
3. Until it is signed, allow it explicitly with
   `allow_loading_unsigned_plugins = tomzone-slurm-panel` under `[plugins]` in
   `grafana.ini`, or
...
before touching a Value mapping by hand, because the rules that colour a Slurm
state fail silently when written the obvious way.
